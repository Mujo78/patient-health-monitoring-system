...
const createToken = async (user, statusCode, res) => {
  ...
  if (info.name) {
    ...
    (inf.first_name = info.first_name), (inf.last_name = info.last_name);
  }
...
const signup = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (verificationToken)
      await new Email(newUser[0], newPatient[0].first_name).send(
        subject,
        "verify-email",
        verifyURL
      );
...
const verifyEmail = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (user.isVerified)
      await new Email(user, patient.first_name).sendWelcomeMessage();
  } catch (err) {
...
const login = asyncHandler(async (req, res) => {
  ...
  if (
    ...
    (user.active === false && user.isVerified === false)
  ) {
    return res.status(404).json("Incorrect password or email!");
  }
...
const forgotPassword = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (resetToken)
      await new Email(user).send(subject, "reset-password", resetURL);
