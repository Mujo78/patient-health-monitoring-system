...
const getMyDepartment = asyncHandler( async (req, res) => {
    ...
    const doctor = await Doctor.findOne({user_id: req.user._id}).select("+department_id")
    if(!doctor) return res.status(400).json("There was an error, please try again later!")

    const department = await Department.findById(doctor.department_id)
    if(!department) return res.status(400).json("There was an error, please try again later!")

    const doctorsFromDepartment = await Doctor.find({department_id: department._id}).sort({first_name: 1})
    if(doctorsFromDepartment.length === 0) return res.status(400).json("There are no doctors registred in database fror this department!")
...
const myDepartmentAppointments = asyncHandler( async (req, res) =>{
    ...
    const appointmentsDay = await Appointment.find({
      ...
      const todayAppointments = appointmentsDay.filter((n) => n.appointment_date >= startOfDay && n.appointment_date <= endOfDay)
