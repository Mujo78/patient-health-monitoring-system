        ...
        const fetchData = async () =>{
            try{
                ...
                if(accessUser && departmentName){
                    const response = await getDepartmentAllInfo(accessUser.token, departmentName)
        ...
        <>
            {loading ? 
                <div className='w-full h-full flex justify-center items-center'>
                    <Spinner size="md" />
                </div> : selectedDepartment ? 
                <div className='divide-y flex flex-col gap-3'>
                    <div className='flex  flex-col'>
                        <div className='flex justify-between'>
                            <p className='text-3xl font-bold'>{selectedDepartment.department.name}</p>
                            <HiOutlineXMark onClick={hideChoosenDepartment} className="cursor-pointer w-[30px] h-[30px]" />
                        </div>
                        <p className='mt-3 text-justify'> {selectedDepartment.department.description}</p>
                        <p className='ml-auto'>Phone number: <Link to={`tel:${selectedDepartment.department.phone_number}`} className='text-blue-700'> +{selectedDepartment.department.phone_number}</Link></p>
                    </div>
                    <div className='flex'>
                    {selectedDepartment.doctors?.length > 0 && typeof selectedDepartment.doctors !== 'string' ? (
                        selectedDepartment.doctors.map((d) => (
                            <Card key={d._id} className="m-3" href={`/appointment/new/${d._id}`}>
