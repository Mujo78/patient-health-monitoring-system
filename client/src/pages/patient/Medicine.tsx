...
import { Medicine as MedicineType, Pharmacy, getMedicine, medicine } from '../../features/medicine/medicineSlice'
        ...
        try {
          if(searchQ){
            ...
            if(categoryQ !== ''){
              ...
              navigate(`/medicine-overview?searchQuery=${searchQuery}&page=${page}&category=${category}`)
            }else{
  ...
  const handleClickSearch = () => {
    if(search !== ''){
      if(search !== searchQ){
        ...
        dispatch(getMedicine({page, searchQuery}))
        navigate(`/medicine-overview?searchQuery=${searchQuery}&page=${page} ${categoryQ ? `&category=${categoryQ}` : ""}`)
      }
    }
  }
          ...
          {loading ?
            ...
            <div className='flex flex-col items-center h-full gap-3 justify-center bg-green-50'>
                ...
                <div className='flex flex-col gap-2'>
                  ...
                  <p>Phone number: <span className='text-blue-700'>+{pharmacy.phone_number}</span></p>
                  <Link to={mailTo}>Email: <span className='underline text-sm text-blue-700'>
                      {pharmacy.user_id.email}
                    </span></Link>
                  <p>Working hours</p>
                  <p className='text-center text-blue-700 font-semibold'>{pharmacy.working_hours}</p>
                </div>
          </div> : <p>err</p>}
        </div>
        {m && 
