...
const PersonalInfoDoc: React.FC = () => {
  ...
  const available_options: OptionDays[] = availableDaysOptions?.filter((m) => !days?.some((d) => d.value === m.value))
          ...
          <div className='w-full flex text-xs gap-1 flex-col h-full justify-center'>
            <div className='w-full flex mt-1'>
              <div className='flex-grow mr-3'>
                ...
                <TextInput id='first_name' className='mt-1' {...register("first_name")} type='text' color={errors.first_name && 'failure'} />
                <ErrorMessage text={errors.first_name?.message} className='text-xs' />
              </div>
              <div className='flex-grow ml-3'>
                <Label htmlFor='last_name' className='text-xs' value='Last Name'  />
                <TextInput id='last_name' className='mt-1' {...register("last_name")} type='text' color={errors.last_name && 'failure'} />
                <ErrorMessage text={errors.last_name?.message} className='text-xs' />
              </div>
            </div>
            <div className='flex justify-between'>
              <div className='flex-grow mr-3'>
                <Label htmlFor='qualification' className='text-xs' value='Qualification'  />
                <TextInput id='qualification' className='mt-1' {...register("qualification")} type='text' color={errors.qualification && 'failure'} />
                <ErrorMessage text={errors.qualification?.message} className='text-xs' />
              </div>
              <div className='flex-grow ml-3'>
            ...
            <div className='flex justify-between'>
              ...
              <div className='w-3/5 pl-3 pr-3'>
                ...
                <TextInput id='address' className='mt-1' {...register("address")} type='text' color={errors.address && 'failure'} />
                <ErrorMessage text={errors.address?.message} className='text-xs' />
              </div>
              <div className='w-2/6'>
                <Label htmlFor='age' className='text-xs' value='Age'  />
                <TextInput id='age' className='mt-1' {...register("age")} type='text' color={errors.age && 'failure'} />
                <ErrorMessage text={errors.age?.message} className='text-xs' />
              </div>
            </div>
            <div className='flex gap-4 justify-center items-center'>
                <div className='w-full'>
                  <Label htmlFor='available_days' className='text-xs' value='Available days'  />
                  <Controller name='available_days' control={control} render={({field}) => (
                     <Select options={available_options} id='available_days' className='mt-1' {...field} isMulti closeMenuOnSelect={false} />
                  )} />
                  <ErrorMessage text={errors.available_days?.message} className='text-xs' />
                </div>
                <div className='w-full'>
                  <Label htmlFor='bio' className='text-xs' value='Bio'  />
                  <Textarea  id='bio' {...register("bio")} color={errors.bio && 'failure'} rows={5} className='text-xs mt-1' />
