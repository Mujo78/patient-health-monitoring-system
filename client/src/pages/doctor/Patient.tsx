              ...
              <div className="flex items-center justify-center mb-12 w-full">
                ...
                <div className="mx-auto">
                  <p className="text-xl font-semibold">
                    {appointments.patient_id.first_name +
                      " " +
              ...
              <div className="flex flex-col h-full justify-center gap-4 flex-grow">
                ...
                <p className="flex text-sm text-gray-500 justify-between">
                  ...
                  <span className="ml-auto text-black">
                    {appointments?.patient_id.date_of_birth.toString()}
                  </span>
                </p>
                <p className="flex text-sm text-gray-500 justify-between">
                  <span>Address :</span>
                ...
                <p className="flex text-sm text-gray-500 justify-between">
                  ...
                  <span className="ml-auto text-black">
                    {appointments?.patient_id.blood_type}
                  </span>
                </p>
                {appointments?.patient_id.height && (
                  <p className="flex text-sm text-gray-500 justify-between">
                    <span>Height (m) :</span>
                    <span className="ml-auto text-black">
                      {Number(appointments?.patient_id.height) / 100}
                    </span>
                  </p>
                )}
                {appointments?.patient_id.weight && (
                  <p className="flex text-sm text-gray-500 justify-between">
                    <span>Weight (kg) :</span>
                    <span className="ml-auto text-black">
                      {appointments?.patient_id.weight}
          ...
          <div className="h-full flex justify-between flex-col px-6 pt-2 w-5/12">
            {Number(page) > appointments.numOfPages ? (
              <div className="h-full flex justify-center items-center">
                {" "}
                <ErrorMessage
                  text={"There are no appointments for this patient."}
                  size="sm"
                />{" "}
              </div>
            ) : appLoading ? (
                  ...
                  <Table>
                    <Table.Body className="divide-y">
                      ...
                      {appointments.data.map((n, index) => (
                        <Table.Row
                          key={n._id}
                          className="mt-12 hover:!bg-blue-100 cursor-pointer"
                          onClick={() => showModal(n)}
                        >
